<template>
    <div style="margin: 10px">
        <div style="width: 100%; display: flex; flex-direction: row">
            <div style="margin-left: 5px">
                <el-upload ref="uploadRef" multiple :auto-upload="false" :show-file-list="false"
                    v-model:file-list="uploadFileList" :on-change="uploadFileChange">
                    <template #trigger>
                        <el-button class="button" size="middle" @click="openUploadFile($event, null)">上传文件</el-button>
                    </template>
                </el-upload>
            </div>
            <div style="margin-left: 20px">
                <el-button size="middle" class="button" @click="openInsertFile($event, null)">
                    添加文件
                </el-button>
            </div>
            <div style="margin-left: 20px">
                <el-button size="middle" class="button" @click="openInsertFolder($event, null)">
                    添加文件夹
                </el-button>
            </div>
        </div>
    ...
    <el-tree
        style="max-width: 600px; background-color: transparent; font-size: 15px;font-family:'Times New Roman', Times, serif;"
        empty-text="没有文件，快去上传吧!" current-node-key="dialogPath" check-on-click-node='true' :allow-drop="allowDrop"
        :allow-drag="allowDrag" :data="dataSource" draggable default-expand-all node-key="id" highlight-current
                        ...
                        <div class="but-list" style="display: flex; flex-direction: column">
                            <div v-if="scope.node.data.type === 'folder'">
                                <el-upload ref="uploadRef" multiple :auto-upload="false" :show-file-list="false"
                                    v-model:file-list="uploadFileList" :on-change="uploadFileChange">
                                    <template #trigger>
                                        <el-button style="width: 124px"
                                            @click="openUploadFile($event, scope.node)">上传文件</el-button>
                                    </template>
                                </el-upload>
                            </div>

                            <div v-if="scope.node.data.type === 'folder'" style="width: 100%">
                                <el-button style="width: 100%" @click="openInsertFile($event, scope.node)">
                                    添加文件
                                </el-button>
                            </div>
                            <div v-if="scope.node.data.type === 'folder'" style="width: 100%">
                                <el-button style="width: 100%"
                                    @click="openInsertFolder($event, scope.node)">添加文件夹</el-button>
                            </div>
                            <div style="width: 100%">
                                <el-button style="width: 100%" @click="openUpdate($event, scope.node)">
                                    修改
                                </el-button>
                            </div>
                            <div style="width: 100%">
                                <el-button style="width: 100%" @click="openDelete($event, scope.node)">删除</el-button>
                            </div>
                        </div>
                    </el-popover>
                </div>
    ...
    <!-- 新增和修改文件火文件夹名称使用 -->
    <el-dialog v-model="fileDialogVisible" :title="dialogTitle" width="500" :before-close="handleClose" draggable
    ...
    <el-dialog v-model="deleteDialogVisible" title="删除" width="500" draggable class="rounded-dialog">
        你确定要删除该文件吗？
        <template #footer>
            <div class="dialog-footer">
                <el-button @click="cancel">取消</el-button>
...
import { ref } from "vue";
import { Folder, Document, Tools } from '@element-plus/icons-vue'
...
const uploadFileChange = (uploadFile) => {
    let data = {
